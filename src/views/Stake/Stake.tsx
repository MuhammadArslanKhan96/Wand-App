...
const Stake: React.FC = () => {
  ...
  const onChangeStake = async (action: string) => {
    ...
    if (action === "stake" && gweiValue.gt(ethers.utils.parseUnits(ohmBalance, "gwei"))) {
      return dispatch(error(t`You cannot stake more than your OPI balance.`));
    }

    if (confirmation === false && action === "unstake" && gweiValue.gt(ethers.utils.parseUnits(sohmBalance, "gwei"))) {
      return dispatch(
        error(
          t`You do not have enough sOPI to complete this transaction.  To unstake from gOHM, please toggle the sohm-gohm switch.`,
        ),
      );
    }
  ...
  const stakingRebasePercentage = 0.8;
  const nextRewardValue = (30700 * stakingRebase) / 100;
    ...
    if (view === 0) {
      if (address && hasAllowance("ohm")) {
        ...
        stakeButtonText = txnButtonText(
          pendingTransactions,
          "staking",
          `${t`Stake to`} ${confirmation ? " gOHM" : " sOPI"}`,
        );
      }
    }
    //If Unstaking Tab
    if (view === 1) {
      if ((address && hasAllowance("sohm") && !confirmation) || (hasAllowance("gohm") && confirmation)) {
        ...
        stakeButtonText = txnButtonText(
          pendingTransactions,
          "unstaking",
          `${t`Unstake from`} ${confirmation ? " gOHM" : " sOPI"}`,
        );
      }
    }
  }

  const getOPIStats = async () => {
    const web3 = await ContractHelper(networkId);
    const sohm = new web3.eth.Contract(sOHMv2, addresses[networkId].SOHM_ADDRESS);
            ...
            <Grid item>
              <MetricCollection>
                ...
                <Metric
                  ...
                  isLoading={stakingAPY && TVL ? false : true}
                />
                {/* <Metric
                  className="stake-index"
                  label={t`Current Index`}
                  metric={`${formattedCurrentIndex} sOPI`}
              ...
              {!address ? (
                <div className="stake-wallet-notification">
                  ...
                  <Typography variant="h6">
                    <Trans>Connect your wallet to stake OPI</Trans>
                  </Typography>
                </div>
              ) : (
                <>
                            ...
                            <Grid item xs={12} sm={8} className="stake-grid-item">
                              <Box mt={"10px"}>
                                <Typography variant="body1" className="stake-note" color="textSecondary">
                                  {view === 0 ? (
                                    <>
                                      <Trans>First time staking</Trans> <b>OPI</b>?
                                      <br />
                                      <Trans>Please approve Olympus Dao to use your</Trans> <b>OPI</b>{" "}
                                      <Trans>for staking</Trans>.
                                    </>
                                  ) : (
                                    <>
                                      <Trans>First time unstaking</Trans> <b>{confirmation ? "gOHM" : "sOPI"}</b>?
                                      <br />
                                      <Trans>Please approve Olympus Dao to use your</Trans>{" "}
                                      <b>{confirmation ? "gOHM" : "sOPI"}</b> <Trans>for unstaking</Trans>.
                                    </>
                                  )}
                                </Typography>
                              </Box>
                  ...
                  <div className="stake-user-data">
                    <DataRow
                      ...
                      id="user-balance"
                      balance={`${trim(Number(ohmBalance), 4)} OPI`}
                    ...
                    <Accordion className="stake-accordion" square defaultExpanded>
                      <AccordionSummary expandIcon={<ExpandMore className="stake-expand" />}>
                        <DataRow
                          ...
                          balance={`${trimmedBalance} sOPI`}
                          isLoading={isAppLoading}
                        />
                      </AccordionSummary>
                      <AccordionDetails>
                        <DataRow
                          title={t`sOPI Balance`}
                          balance={`${trim(Number(sohmBalance), 4)} sOPI`}
                          indented
                          isLoading={isAppLoading}
                        />
                        {/* <DataRow
                        ...
                        {Number(sohmV1Balance) > 0.00009 && (
                          <DataRow
                            title={`${t`sOPI Balance`} (v1)`}
                            balance={`${trim(Number(sohmV1Balance), 4)} sOPI (v1)`}
                            indented
                            isLoading={isAppLoading}
                          />
                        )}
                        {Number(wsohmBalance) > 0.00009 && (
                          <DataRow
                            title={`${t`wsOPI Balance`} (v1)`}
                            balance={`${trim(Number(wsohmBalance), 4)} wsOPI (v1)`}
                            isLoading={isAppLoading}
                            indented
                          />
                        )}
                        {Number(fiatDaowsohmBalance) > 0.00009 && (
                          <DataRow
                            title={t`wsOPI Balance in FiatDAO (v1)`}
                            balance={`${trim(Number(fiatDaowsohmBalance), 4)} wsOPI (v1)`}
                            isLoading={isAppLoading}
                            indented
                          />
                        )}
                    ...
                    <DataRow
                      ...
                      balance={`${trim(Number(fiveDayRate), 4)}%`}
                      isLoading={isAppLoading}
                    />
                  </div>
                </>
