...
import { OlympusStakingv2__factory, SOhmv2 } from "../typechain";
...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: IBaseAsyncThunk, { dispatch }) => {
    ...
    // const treasuryContract = new ethers.Contract(addresses[networkID].TREASURY_ADDRESS as string, Treasury, signer);
    ...
    // Calculating staking
    try {
      // const epochs = await stakingContracts.methods.epoch().call();
      // console.log({ epochs });
      const epoch = await stakingContract.epoch();
      const secondsToEpoch = Number(await stakingContract.secondsToNextEpoch());
      const stakingReward = epoch.distribute;
      const circ = await sohmMainContract.circulatingSupply();
      // const circs = await sohm.methods.circulatingSupply().call();
      const stakingRebase = Number(stakingReward.toString()) / Number(circ.toString());
      const fiveDayRate = 30700 * 0.8 * 3 * 5;
      const stakingAPY = 30700 * 0.8 * 3 * 30 * 12;

      // const ep = await stakingContractV1.epoch();
      // const sec = await ohmContract.totalReserves();
      // console.log({ sec, ep });

      // Current index
      const currentIndex = await stakingContract.index();
      const currentIndexV1 = await stakingContract.index();
      return {
